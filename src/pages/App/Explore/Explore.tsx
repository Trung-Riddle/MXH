...
import { toggleOpenModalExplore, toggleOpenModalUpload } from 'src/store/slices/modal/explore.slice'
...
const Explore = () => {
  ...
  useEffect(() => {
    const fetchapi = async () => {
      const results = await fetch(`https://api.cloudinary.com/v1_1/dyfyxrbm6/resources/image`, {
        headers: {
          Authorization: `Basic ${Buffer.from('845178792751456:LZnsLAk6V3bvdCXpStHkRM4bEe4').toString('base64')}`
        }
      }).then((r) => r.json())
    }
  ...
  return (
    <>
      ...
      <div className='columns-5 px-6 overflow-auto'>
        <button
          onClick={() => setSearchParams({ modal_id: 'vqv1yhjoa6o1ttiyslw8' })}
        ...
        >
          <div className='relative group' onMouseOver={handleMouseOver} onMouseOut={handleMouseOut}>
            <video ref={ref} muted={muted} playsInline>
              ...
              <source src='https://res.cloudinary.com/dyfyxrbm6/video/upload/q_auto/v1/videos/vqv1yhjoa6o1ttiyslw8.mp4?_a=BATASxAA0' />
            </video>
          ...
          <div className='bg-light dark:bg-dark p-3'>
            <button onClick={() => dispatch(toggleOpenModalExplore())}>
