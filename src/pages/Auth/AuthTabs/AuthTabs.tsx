    ...
    <div className='w-full h-screen flex'>
      ...
      <div className='w-2/3'>
        <div className='wrapper'>
          <div className='logo mt-20 flex items-center gap-2 justify-center'>
            <svg xmlns='http://www.w3.org/2000/svg' width={82} height={76} viewBox='0 0 114 111' fill='none'>
              <path
            ...
            <span className='font-bold text-7xl style-main block-select'>lime8</span>
          </div>
          <h2 className='text-center font-bold text-3xl text-black-1 my-5'>{!tab ? 'Login' : 'Register'}</h2>
          <div className='flex flex-col items-center w-3/4 mx-auto'>
            {!tab ? <Login /> : <Register />}
            <p>
              <span className='mr-1 text-black-1 font-bold'>
                {!tab ? 'Dont have an account?' : 'Already an account?'}
              </span>
              <button className='style-main font-bold' onClick={toggle}>
                {!tab ? 'Register' : 'Login'}
              </button>
            </p>
          </div>
